...
- Publish at least one Agent Skills-compatible repository-root package named `skill/<toolname>-cli/`.
- Keep the package root deterministic: `<repo-root>/skill/<toolname>-cli/SKILL.md`.
- Keep frontmatter `name` aligned with the folder name (for example `toolname-cli`).
